function [handles, X] = barweb(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type)
...
% Updated by Alice in January 2017 to plot error bars
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	if ~isempty(bw_colormap)
		colormap(gca,bw_colormap);
        for n = 1:length(bars)
            bars(n).EdgeColor = [0 0 0];
            bars(n).LineWidth = 3;
            bars(n).FaceColor = bw_colormap;
        end
	else
		colormap(gca,jet);
        for n = 1:length(bars)
	...
	for i = 1:numbars
        ...
        bars(i).EdgeAlpha = 1;
        x = bars(i).XData + bars(i).XOffset;
        X{i} = bars(i).XData + bars(i).XOffset;
%         width = bars.BarWidth;
%         if i == 1
%             x = x - (width/4)*ones(1,length(x));
%         else
%             x = x + (width/4)*ones(1,length(x));
%         end
% 		x = get(get(handles.bars(i),'children'), 'xdata');
		%x = mean(x([1 3],:));
		handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,i), 'k', 'linestyle', 'none', 'linewidth', 3);
	...
	if strcmp(legend_type, 'axis')
		for i = 1:numbars
			...
			for j = 1:length(xdata)
				text(xdata(j),  -0.03*ymax*1.1, bw_legend(i), 'Rotation', 60, 'fontsize', 14, 'fontweight', 'bold', 'HorizontalAlignment', 'right');
			end
		end
	...
	if ~isempty(bw_xlabel)
		xlabel(bw_xlabel, 'fontsize',20);
	end
	if ~isempty(bw_ylabel)
		ylabel(bw_ylabel, 'fontsize',20);
	end
	
	set(gca, 'xticklabel', groupnames, 'box', 'on', 'ticklength', [0 0], 'fontsize', 8, 'xtick',1:numgroups, 'linewidth', 1,'xgrid','off','ygrid','off');
